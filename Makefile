...
CFLAGS = -I/usr/local/opt/readline/include/ -Wall -Werror -Wextra -g

VALGRIND = valgrind --leak-check=full --show-leak-kinds=all -s --track-fds=yes
# CFLAGS = -Wall -Werror -Wextra -fsanitize=address

#LIB_RL_FLAG = -L/usr/include -lreadline change to 
LIB_RL_FLAG = -lreadline -L/usr/local/opt/readline/lib/
#LIB_RL_FLAG = -L /usr/local/opt/readline/lib -lreadline -I/usr/local/opt/readline/
...
## FOR PARSER
PARSER_PATH = ./parser/
PARSER_FILE = util.c expand_util.c parser.c print_tok.c set_mode.c expand.c error_parser.c ft_split_special.c ft_split_pipe.c ft_split_sp_util.c
